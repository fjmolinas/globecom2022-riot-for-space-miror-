...
O%n such a cubesat, the power available is only enough for one active payload at a time. 
...
%divided in 8 slots.% is divided in 8 slots.
%The Thingsat payload occupies one slot out of 8 and would be turned ON 12.5\% of the time. 
%This is obviously an average since the schedule to turn on payloads relies on multiple factors (mission specificities, regulations, battery level). 
